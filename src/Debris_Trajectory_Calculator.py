...
terrain_ft = 617.0
terrain_m = terrain_ft * 0.3048

# Final two lead-in points (lon, lat, alt metres AMSL)
penultimate_lon = -89.826757
penultimate_lat = 38.54611918

final_lon = -89.82636619
final_lat = 38.54615159
final_alt = 355.10400000000004  # metres AMSL

#final output file directory
output_file = "./output/file.kml"
...
# Altitude offset so that at t=0: (terrain + z_sim + offset) == final_alt (AMSL)
alt_m_sim_start = alt_ft_input * 0.3048
alt_offset = final_alt - (terrain_m + alt_m_sim_start)
...
for _, row in df.iterrows():
    ...
    lat = final_lat + dlat
    lon = final_lon + dlon
    alt_abs = terrain_m + z + alt_offset
